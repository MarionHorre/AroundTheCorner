class HousesController < ApplicationController
  ...
  def lilloise_url
    html_content = URI.open("https://lalilloise-immobilier.fr/annonces/")
    @url = Nokogiri::HTML(html_content)
  end
  ...
  def scrapping_lilloise
    html_doc.search('.card').each do |element|
      ...
      url_image = element.search(".attachment-annonce-thumb").attr('data-src').value.gsub(/400x200/, '640x480')
      House.create[name: name, description: description, url_image: url_image, square_meter: square_meter, room_number: room_number, price: price, link: link]
    end
  end
end
